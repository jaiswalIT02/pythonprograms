...
def callback(r,c):
    ...
    if player=='X' and status[r][c]==0 and stop_game==False:
        b[r][c].configure(text='X' ,fg=='blue', bg='white')
        states[r][c]='X'
        player='O'

    if player=='O' and status[r][c]==0 and stop_game==False:
...
def check_for_winner():
    ...
    for i in range(3):
        ...
        if states[0][0]==states[1][2]==states==[2][2]!=0:
...
for i in range(3):
    for j in range(3):
        b[i][j]=Button(font=("Arial",60),width=4,bg=("poederblue",
