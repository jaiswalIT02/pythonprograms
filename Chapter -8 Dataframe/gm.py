...
#If the players want to save the game in that position:-        
        n=int(input("enter 0 to save {0} move".format(player)))  
        if n==0:
            #Do the saving part
...
def readexcel(excel):
    ...
    return df
df=readexcel("F:\\Excel2\\Tictactoe.xlsx")
print(df)


print("____________Welcome to Tictactoe Game______________\n")
players=[0,0]
print("Do you want to play Saved Game or New Game :-")
savedgame=int(input("Press '0' to play savedgame or '1' to new game:"))
if savedgame==0:
    savedgame=input("Enter the saved file name ")
    filename="F:\\game\\" + savedgame + ".xlsx"
    players[0],players[1],tictactoe,currentplayer,gametstaus=ex.ReadFromExcel(filename)
    tictactoe=ex.strtolist(tictactoe)
...
while True:   
        ...
        if move==9:     #This line will processing to saving game
            print("Do the saving game")       
            filename= input("Enter the file name to save the game:") #here the users have to give a file name to save
            
            filename="F:\\game\\" + filename + ".xlsx"   #This is a file location and file extension of whose file would be given by users 
            player1=players[0]
            player2=players[1]
            ex.SaveToExcel(player1,player2,ex.listtostr(tictactoe),currentplayer,0,filename)
